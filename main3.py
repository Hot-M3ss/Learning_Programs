...
def run_game():
    ...
    while playing_blackjack == (True):
        if dealt_hand >= 3:
            ...
            deal_cards()
            if dealt_hand == 2:
                has_blackjack(player_hand_suites)
...
def deal_cards():
    ...
    if dealt_hand < 2:
        ...
        for card in player_hand_suites:
            if card in current_game_deck:
                current_game_deck.remove(card)
        print(f'Player {player_hand_suites}')
    elif dealt_hand >= 2:
        print('temp')
        dealt_hand+=1
    dealt_hand+=1
...
def has_blackjack(card):
    global FACE_CARDS, player_hand_values
    for x in card:
        if x[2:4] == 'A':
            player_hand_values.append(int(11))
            print(f'what? 1 {player_hand_values}')
        elif x[2:4] in FACE_CARDS: 
            player_hand_values.append(int(10))
            print(f'what? 2 {player_hand_values}')
        else:
            player_hand_values.append(int(x[2:4]))
            print(f'what? 3 {player_hand_values}')
